...
case $choice in
    1)
        ...
        cd source && python3 programme_antenne_unifie.py emetteur
        ;;
    2)
        echo "Lancement du récepteur en mode réel..."
        cd source && python3 programme_antenne_unifie.py recepteur
        ;;
    3)
        echo "Lancement de l'émetteur en simulation..."
        cd source && python3 programme_antenne_unifie.py emetteur --simulation
        ;;
    4)
        echo "Lancement du récepteur en simulation..."
        cd source && python3 programme_antenne_unifie.py recepteur --simulation
        ;;
    5)
        echo "Test du fichier émetteur ESP32..."
        cd source && python3 emetteur_esp32.py
        ;;
    6)
        echo "Test du fichier récepteur ESP32..."
        cd source && python3 recepteur_esp32.py
        ;;
    *)
        echo "Choix invalide !"
        exit 1
